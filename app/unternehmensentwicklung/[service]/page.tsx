...
export default async function DevelopmentServicePage({ params }: Props) {
  ...
  return <div className={styles.page}>
    ...
    <ProofStrip />
    <section id="ergebnis" className={styles.section}><div className={styles.container}><div className={styles.sectionHeading}><div><p className={styles.eyebrow}>KLARER AUFTRAG. KONKRETES ERGEBNIS.</p><h2>Was Sie danach<br /><span>in der Hand haben.</span></h2></div><p>{phase.description}</p></div><div className={styles.deliverableGrid}>{phase.deliverables.map((item, index) => <article key={item.title}><span className={styles.micro}>0{index + 1} / ERGEBNIS</span><h3>{item.title}</h3><p>{item.text}</p></article>)}</div></div></section>

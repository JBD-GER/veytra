...
export default async function DevelopmentServicePage({ params }: Props) {
  ...
  return <div className={styles.page}>
      ...
      <div className={styles.container}>
        ...
        <section id="preis" className={styles.servicePrice} aria-labelledby="service-price-title">
          ...
          <div className={styles.servicePriceOffer}>
            ...
            <Link href="/kontakt" className={styles.primaryButton}>{contactCtaLabel} <Arrow diagonal /></Link>
          </div>
        </section>
      </div>
    </section>
    ...
    <section className={styles.serviceDecision}><div className={styles.container}><p className={styles.eyebrow}>DIE FRAGE HINTER DER ANALYSE</p><h2>{phase.question}</h2><p>{phase.answer}</p><Link href="/kontakt" className={styles.primaryButton}>{contactCtaLabel} <Arrow diagonal /></Link>{phase.id === "validate" && <p className={styles.serviceDisclaimer}>Das VEYTRA Rating ist eine strukturierte Entscheidungsgrundlage. Es garantiert keinen wirtschaftlichen Erfolg.</p>}{phase.id === "improve" && <p className={styles.serviceDisclaimer}>Ergebnisverbesserungen hängen von Datenlage, Markt und Umsetzung ab. Ausgewiesene Potenziale sind Szenarien und keine Zusagen.</p>}</div></section>
